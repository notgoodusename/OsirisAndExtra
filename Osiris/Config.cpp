...
static void from_json(const json& j, Config::Visuals::Weather& r)
{
    read(j, "Enabled", r.enabled);
    read(j, "Type", r.type);
...
static void to_json(json& j, const Config::Visuals::Weather& o, const Config::Visuals::Weather& dummy)
{
    WRITE("Enabled", enabled);
    WRITE("Type", type);
    WRITE("Length", length);
