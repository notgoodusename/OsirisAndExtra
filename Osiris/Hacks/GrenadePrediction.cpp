...
void NadePrediction::draw() noexcept
{
	...
	static const auto redColor = ImGui::GetColorU32(ImVec4(1.f, 0.23f, 0.24f, 1.f));
	...
	for (auto& point : dmgPoints)
	{
		...
		drawList->AddText({ point.first.x - horizontalOffset + 1.0f, point.first.y - verticalOffset + 1.0f }, redColor & IM_COL32_A_MASK, point.second.c_str());
		drawList->AddText({ point.first.x - horizontalOffset, point.first.y - verticalOffset }, redColor, point.second.c_str());
